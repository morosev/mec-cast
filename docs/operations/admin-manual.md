...
| `ptp.reliable: false` on a lab run | nothing is disciplining `CLOCK_REALTIME` from the PHC, **or** the node could not open its PHC — check the node log, which names which | `bash deploy/lab/ptp/verify-ptp.sh` |
